...
void configurar_lorawan(TConfig_LoRaWAN *pt_lorawan)
{
    ...
    //ESP_LOGI(TAG_LOGS_LORAWAN, "Reseta modem ...");
    //memset(cmd_at, 0x00, sizeof(cmd_at));
    //snprintf(cmd_at, sizeof(cmd_at), "ATZ");
    //envia_comando_reset(cmd_at);
...
void envia_payload_lorawan(char *pt_payload)
{
    ...
    snprintf(cmd_envio_payload, sizeof(cmd_envio_payload), "AT+SENDB=3:%s", pt_payload);
    ESP_LOGI(TAG_LOGS_LORAWAN, "Comando para envio do payload: %s . Tamanho do payload: %d bytes", cmd_envio_payload, (strlen(pt_payload)/2));
    envia_comando_uart(cmd_envio_payload, strlen(cmd_envio_payload));
}

/* Função: envia comando de reset
 * Parâmetros: comando
 * Retorno: nenhum
 */
void envia_comando_reset(char *pt_comando)
{
    char cmd_envio_reset[TAM_MAX_CMD_AT] = {0};

    snprintf(cmd_envio_reset, sizeof(cmd_envio_reset), "%s", pt_comando);
    ESP_LOGI(TAG_LOGS_LORAWAN, "Comando para envio do payload: %s", cmd_envio_reset);
    envia_comando_uart(cmd_envio_reset, strlen(cmd_envio_reset));

    /* Limpa a UART */
    while (uart_read_bytes(SENS_LORAWAN_UART_PORT_NUM,
                           (char *)buffer_recepcao, (BUF_SIZE - 1),
                           TEMPO_ENTRE_COMANDOS_AT / portTICK_PERIOD_MS) > 0)
    {
